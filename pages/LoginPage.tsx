        ...
        >
            ...
            <div className="mx-auto w-full max-w-md rounded-2xl bg-brand-dark/80 backdrop-blur-sm p-4 md:p-8 z-10 border border-brand-gold/30 shadow-2xl shadow-brand-gold/10">
                ...
                <h2 className="text-xl font-bold text-white font-serif">
                    {needsAdminAccess ? t('login.adminTitle') : t('login.title')}
                </h2>
                <p className="mt-2 max-w-sm text-sm text-brand-accent">
                    {needsAdminAccess ? t('login.adminSubtitle') : t('login.subtitle')}
                </p>
                ...
                <form className="my-8" onSubmit={handleSubmit}>
                    ...
                    <LabelInputContainer className="mb-4">
                        <Label htmlFor="email" className="text-brand-accent">{t('login.emailLabel')}</Label>
                        <Input
                            id="email"
                    ...
                    <LabelInputContainer className="mb-8">
                        <Label htmlFor="password" className="text-brand-accent">{t('login.passwordLabel')}</Label>
                        <Input
                            id="password"
                            placeholder="••••••••"
                            type="password"
                    ...
                    {!needsAdminAccess && (
                        <p className="mt-8 text-center text-sm text-brand-accent/80">
                            {t('login.noAccount')}{' '}
                            <a onClick={() => onNavigate('register')} className="font-medium text-brand-gold hover:text-brand-accent cursor-pointer transition-colors">
